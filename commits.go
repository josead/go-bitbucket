...
func (cm *Commits) CreateBuildStatus(cmo *CommitsOptions, body string) interface{} {
	url := cm.c.requestUrl("/repositories/%s/%s/commit/%s/statuses/build", cmo.Owner, cmo.Repo_slug, cmo.Revision)
	return cm.c.execute("POST", url, body)
}

func (cm *Commits) UpdateBuildStatus(cmo *CommitsOptions, body string) interface{} {
	url := cm.c.requestUrl("/repositories/%s/%s/commit/%s/statuses/build/%s", cmo.Owner, cmo.Repo_slug, cmo.Revision, cmo.Key)
	return cm.c.execute("PUT", url, body)
}
